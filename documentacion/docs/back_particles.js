document.addEventListener("DOMContentLoaded", () => {
  console.log("Ragnos Particles: Script loaded!");

  /* --- Particles Background (Ported from main.js) --- */

  // Try to get existing canvas or create one
  let canvas = document.getElementById("particles-canvas");
  if (!canvas) {
    canvas = document.createElement("canvas");
    canvas.id = "particles-canvas";
    // Style as fixed background
    canvas.style.position = "fixed";
    canvas.style.top = "0";
    canvas.style.left = "0";
    canvas.style.width = "100%";
    canvas.style.height = "100%";
    // z-index -1 might be hidden by body background in some themes.
    // Trying 0 with pointer-events none to sit above body background but below content if content is stacked.
    // If still hidden, we can try higher z-index, but mkdocs content usually has no z-index context.
