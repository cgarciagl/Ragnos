...
abstract class RReportLib
{
    ...
    private function applyFiltersAndOrder(Model $modelo): void
    {
        $ordertoapply = []; //
        for ($i = 1; $i <= 3; $i++) { //
            $nivel          = $this->request->getPost("nivel{$i}", FILTER_SANITIZE_STRING); // Usar $this->request
            $filter         = $this->request->getPost("filter{$i}", FILTER_SANITIZE_STRING); // Usar $this->request
            $ragnosIdFilter = $this->request->getPost("Ragnos_id_filter{$i}"); // Usar $this->request
    ...
    {
        $reportType = $this->request->getPost('typeofreport', FILTER_SANITIZE_STRING); //
