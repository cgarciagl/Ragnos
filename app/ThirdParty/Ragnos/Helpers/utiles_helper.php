...
if (!function_exists('removeNewLines')) {
...
if (!function_exists('sessionObject')) {
...
if (!function_exists('redirectAndDie')) {
    function redirectAndDie($url, $statusCode = 302)
    {
        $url = strpos($url, 'http') === 0 ? $url : site_url($url);

        if (ENVIRONMENT === 'testing') {
            $redirectResponse = redirect()->to($url, null, $statusCode);
            throw new \CodeIgniter\HTTP\Exceptions\RedirectException($redirectResponse);
        }
...
if (!function_exists('checkApiCall')) {
...
if (!function_exists('queryToAssocArray')) {
    /**
...
if (!function_exists('queryToAssocArrayParams')) {
    /**
